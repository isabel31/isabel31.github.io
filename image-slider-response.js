  ...
  class ImageSliderResponsePlugin {
      ...
      trial(display_element, trial) {
          ...
          window.addEventListener('click', function (e) {
              that.clicks.push({ x: e.pageX, y: e.pageY });
          });
          const end_trial = () => {
              this.jsPsych.pluginAPI.clearAllTimeouts();
              // assumes only one slider!
              var slider_element = document.getElementById("jspsych-image-slider-response-response");
              // won't work for display: inline, but that would be weird (jsPsych default is block)
              var style = getComputedStyle(slider_element);
              var height = slider_element.clientHeight;
              var width = slider_element.clientWidth;
              height -= parseFloat(style.paddingTop) + parseFloat(style.paddingBottom);
              width -= parseFloat(style.paddingLeft) + parseFloat(style.paddingRight);
              var slider_position = {
                  top: slider_element.getBoundingClientRect().top + slider_element.scrollTop,
                  left: slider_element.getBoundingClientRect().left + slider_element.scrollTop,
                  width: width,
                  height: height
              };
              // save data
              var trialdata = {
                  ...
                  slider_position_top: slider_position.top,
                  slider_position_left: slider_position.left,
                  slider_position_width: slider_position.width,
                  slider_position_height: slider_position.height,
                  clicks: this.clicks
              };
