var jsPsychSurveyLikertWithClicks = (function (jspsych) {
  'use strict';

  const info = {
      name: "survey-likert-with-clicks",
      parameters: {
   ...
   */
  class SurveyLikertWithClicksPlugin {
      constructor(jsPsych) {
      ...
      trial(display_element, trial) {
          ...
          window.addEventListener('click', function (e) {
              that.clicks.push([e.pageX, e.pageY]);
          });
          display_element.querySelector("#jspsych-survey-likert-form").addEventListener("submit", (e) => {
              ...
              // assumes only one likert!
              var likert_element = document.getElementsByClassName("jspsych-survey-likert-opts")[0];
              // won't work for display: inline, but that would be weird (jsPsych default is block)
              var style = getComputedStyle(likert_element);
              var height = likert_element.clientHeight;
              var width = likert_element.clientWidth;
              height -= parseFloat(style.paddingTop) + parseFloat(style.paddingBottom);
              width -= parseFloat(style.paddingLeft) + parseFloat(style.paddingRight);
              var likert_position = {
                  top: likert_element.getBoundingClientRect().top + likert_element.scrollTop,
                  left: likert_element.getBoundingClientRect().left + likert_element.scrollTop,
                  width: width,
                  height: height
              };
              // save data
              var trial_data = {
                  ...
                  clicks: this.clicks,
                  likert_position: likert_position
              };
  ...
  SurveyLikertWithClicksPlugin.info = info;

  return SurveyLikertWithClicksPlugin;
